...
int
channel_pulse(int channel, int gpio, int width_start, int width, int add)
{
    int i;
    uint32_t phys_gpclr0 = 0x7e200000 + 0x28;
    ...
    log_debug("%s_channel_pulse: channel=%d, gpio=%d, start=%d, width=%d\n", add?"add":"remove", channel, gpio, width_start, width);
    if (!channels[channel].virtbase)
        return fatal("Error: channel %d has not been initialized with 'init_channel(..)'\n", channel);
...
void *
add_measure(int channel, int pos)
{
    uint32_t phys_gpin = 0x7e200000 + 0x34;
    dma_cb_t *cbp = (dma_cb_t *) get_cb(channel) + (pos * 2);
    uint32_t *sample = (uint32_t *) channels[channel].virtbase;

    log_debug("add_channel_input: channel=%d, pos=%d\n", channel, pos);
    if (!channels[channel].virtbase) {
        log_debug("Error: channel %d has not been initialized with 'init_channel(..)'\n", channel);
        return NULL;
    }
    if (pos > channels[channel].width_max || pos < 0) {
        log_debug("Error: cannot add input to channel %d: width_start+width exceed pos of %d\n", channels[channel].width_max);
        return NULL;
    }
    // add gpio measure
    //cbp->dst = cbp->src;
